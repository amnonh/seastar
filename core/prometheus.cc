...
future<> get_text_representation(output_stream<char>& out, const metrics_families_per_shard& families, const config& ctx) {
    return do_with(metric_family_range(families), bool(false),
...
future<> get_protobuf_representation(output_stream<char>& out, const metrics_families_per_shard& families, const config& ctx) {
...
public:
    ...
    future<std::unique_ptr<httpd::reply>> handle(const sstring& path,
        ...
        rep->write_body((text) ? "txt" : "proto", [this, text] (auto&& s) {
            return do_with(metrics_families_per_shard(), output_stream<char>(std::move(s)),
                    [this, text] (auto& families, auto& s) mutable {
                return get_map_value(families).then([&s, &families, this, text]() mutable {
                    return (text) ? get_text_representation(s, families, _ctx) :
                            get_protobuf_representation(s, families, _ctx);
                }).then([&s] () mutable {
                    return s.close();
                });
            });
